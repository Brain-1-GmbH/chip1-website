        ...
        <div className="flex flex-col gap-6 md:hidden">
          ...
          <div className="flex items-start gap-6 w-full">
            ...
            <div className="flex flex-col gap-4 flex-1">
              <h3
                className="text-lg font-semibold text-[#e5e5e7] leading-[1.4]"
              ...
              <nav className="flex flex-col">
                {quickLinks.map((link) => (
                  <a
                    key={link.label}
                    href={link.href}
                    className="py-2 text-base font-medium text-[#e5e5e7] hover:text-[#99c221] transition-colors"
                  >
                    {link.label}
                  </a>
                ))}
              ...
              <nav className="flex flex-col">
                {socialLinks.map((link) => (
                  <a
                    key={link.label}
                    href={link.href}
                    className="flex items-center gap-2.5 py-2 text-base font-medium text-[#e5e5e7] hover:text-[#99c221] transition-colors"
                  >
                    <link.icon size={20} />
                    {link.label}
                  </a>
            ...
            <div className="flex flex-wrap gap-4">
              {legalLinks.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  className="text-sm text-[#e5e5e7] underline hover:text-[#99c221] transition-colors"
                >
                  {link.label}
                </a>
              ))}
            </div>
            <p className="text-sm text-[#e5e5e7] leading-[1.5]">
        ...
        <div className="hidden md:flex flex-col gap-6">
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex flex-col gap-6 w-[420px]">
              {/* Logo */}
              <a href="/" className="flex items-center">
                <img src={logoImg} alt="Chip1 Logo" className="h-12 w-auto" />
              </a>
