...
import {
  ...
  fetchSemiconductorCategories,
} from "../../services/categoryApi";
import type { Category } from "../../types/category";
import { CATEGORY_L1_KEYS } from "../../types/category";

// Default images for category cards (will cycle through based on index)
...
// Type for displaying categories with UI-specific fields
...
const transformCategories = (categories: Category[]): DisplayCategory[] => {
  return categories.map((cat, index) => ({
    title: cat.name.value,
    count: cat.count,
    imageIndex: index % categoryCardImages.length,
    categoryKey: cat.name.key,
    subcategories: cat.subCategories.map((sub) => ({
      name: sub.name.value,
      count: sub.count,
      categoryKey: sub.name.key,
    })),
  }));
};
...
export const ByCategoryPage: React.FC = () => {
  ...
  // API data states
  const [hardwareCategories, setHardwareCategories] = useState<DisplayCategory[]>([]);
  const [semiconductorCategories, setSemiconductorCategories] = useState<DisplayCategory[]>([]);
  ...
  const loadCategories = useCallback(async () => {
    ...
    setError(null);
    try {
      const [hardwareResponse, semiconductorResponse] = await Promise.all([
        fetchHardwareCategories(),
        fetchSemiconductorCategories(),
      ]);
      setHardwareCategories(transformCategories(hardwareResponse.data));
      setSemiconductorCategories(transformCategories(semiconductorResponse.data));
    } catch (err) {
  ...
  const categoryL1Key = activeTab === "hardware" 
  ...
  const handleSubcategoryClick = (
    ...
    // Use slugs for URL display but pass keys for API filtering
    ...
    navigate(`/category/${activeTab}/${categorySlug}/${subcategorySlug}`, {
      state: {
        categoryL1Key,
        categoryL2Key: categoryKey,
        categoryL3Key: subcategoryKey,
      },
    });
  };
          ...
          {loading && (
            <div className="flex items-center justify-center py-24">
              <div className="flex flex-col items-center gap-4">
                <div className="w-12 h-12 border-4 border-[#99c221] border-t-transparent rounded-full animate-spin" />
                <p
                  className="text-[#8e8e8f] text-lg"
                  style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
                >
                  Loading categories...
                </p>
            ...
            <div className="flex items-center justify-center py-24">
              <div className="flex flex-col items-center gap-4">
                ...
                <button
                  ...
                  className="px-6 py-2 text-[#0F0F0F] font-medium rounded-full"
                  style={{
                    fontFamily: "'Plus Jakarta Sans', sans-serif",
                    background: "#99C221",
                  }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {categories.map((category) => (
                ...
                >
                  ...
                  <img
                    src={categoryCardImages[category.imageIndex]}
                    alt={category.title}
